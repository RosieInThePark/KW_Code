...
IMPLEMENT_DYNCREATE(CMFCApplication1Doc, CDocument)
	...
	void CMFCApplication1Doc::set_face(Face * head) {
		int (*temp)[3][3];

		temp = (int(*)[3][3])head->get_value();
		for(int i=0;i<3;i++) {
			for(int j=0;j<3;j++) {
				m_face[0][i][j] = (*temp)[i][j];
			}
		}
		temp = (int(*)[3][3])head->get_up()->get_value();
		for(int i=0;i<3;i++) {
			for(int j=0;j<3;j++) {
				m_face[1][i][j] = (*temp)[i][j];
			}
		}
		temp = (int(*)[3][3])head->get_next()->get_value();
		for(int i=0;i<3;i++) {
			for(int j=0;j<3;j++) {
				m_face[2][i][j] = (*temp)[i][j];
			}
		}
	}
